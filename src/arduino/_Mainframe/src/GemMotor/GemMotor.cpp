...
void GemMotor::Update(float velocity) {
  int currentRPM        = GemMotor::GetRpm();
  float currentVelocity = (float)currentRPM * RPM_TO_KMH;  // TODO: verify that this calculation returns a float (RPM_TO_KMH constant, previously resulted in faulty calculations)
  float velocityChange  = velocity - currentVelocity;
